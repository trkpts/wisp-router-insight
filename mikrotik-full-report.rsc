# Comprehensive MikroTik Monitoring Script for WISP
# Collects bandwidth, WiFi info, SSIDs, and passwords
...
:global getInterfaceStats do={
    ...
    :foreach i in=[/interface find] do={
        ...
        :local rxByte [/interface monitor-traffic $i once do={:set rxByte \$rxByte}];
        :local txByte [/interface monitor-traffic $i once do={:set txByte \$txByte}];
    ...
    :return [:pick $result 0 ([:len $result] - 1)]; # Remove trailing comma
};

# Function to collect wireless info (SSID, passwords, signal strength)
:global getWirelessInfo do={
    ...
    :foreach i in=[/interface wireless find] do={
        ...
        # Get associated stations (clients)
        :local clients [];
        :foreach station in=[/interface wireless registration-table find interface=$name] do={
        ...
        :if ([:len $clients] > 0) do={
            :set clients [:pick $clients 0 ([:len $clients] - 1)]; # Remove trailing comma
        };
    ...
    # Get wireless security profiles to extract passwords
    :foreach i in=[/interface wireless security-profiles find] do={
        :local name [/interface wireless security-profiles get $i name];
        :local authenticationType [/interface wireless security-profiles get $i authentication-types];
        ...
        :if ($passphrase != "") do={
            ...
            :set result ($result . "\"authentication_types\":\"$authenticationType\",");
            :set result ($result . "\"mode\":\"$mode\",");
    ...
    :if ([:len $result] > 0) do={
        :set result [:pick $result 0 ([:len $result] - 1)]; # Remove trailing comma
    };
    :return $result;
};
...
:global sendReport do={
    ...
    :log info "WISP Monitoring: Sent data to server. Response: $response";
};
...
:if {[/system scheduler find name="wisp-monitoring"]} do={
