...
:global getInterfaceStats do={
    ...
    :foreach i in=[/interface find] do={
        ...
        # Get interface statistics from /interface print
        :local interfaceInfo [/interface print terse=$i];
        :local rxByte 0;
        :local txByte 0;
        
        # Parse the interface info to extract byte counts
        # Format varies, but typically contains rx/tx byte information
        :do {
            :local stats [/interface monitor $i once];
            # Extract rx-byte and tx-byte from the statistics
            :set rxByte [:tonum ([/interface get $i value-name=rx-byte])];
            :set txByte [:tonum ([/interface get $i value-name=tx-byte])];
        } on-error={
            # Alternative method if direct access doesn't work
