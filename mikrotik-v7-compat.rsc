# MikroTik Script for WISP Monitoring (RouterOS v7 Compatible)
...
:global getInterfaceStats do={
    ...
    :foreach i in=[/interface find] do={
        ...
        # Get traffic data using interface monitor (RouterOS v7 compatible)
        :local rxByte 0;
        :local txByte 0;
        :do {
            :local trafficData [/interface monitor-traffic numbers=$i once];
            :set rxByte [:tonum ($trafficData->"rx-byte")];
            :set txByte [:tonum ($trafficData->"tx-byte")];
        } on-error={
            # Fallback if monitor-traffic fails
            :set rxByte 0;
            :set txByte 0;
        };
